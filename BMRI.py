...
summary_df = pd.DataFrame(summary_stats, columns=["Variable", "N", "Mean", "Std Dev", "Min", "Max", "Missing", "Outliers"])
...
assump_df = pd.DataFrame(results_table, columns=["Assumption", "Test", "Result", "Interpretation", "Status"])
...
simple_df = pd.DataFrame(simple_reg_results, columns=["Variable", "Coefficient (β)", "Std Error", "t-value", "p-value", "95% CI", "Significance"])
print("\n=== Simple Regression Per Variable ===")
print(simple_df.to_string(index=False))

# ===== 9. MULTIPLE REGRESSION COEFFICIENTS =====
multi_results = []
...
multi_df = pd.DataFrame(multi_results, columns=["Variable", "Coefficient (β)", "Std Error", "t-value", "p-value", "95% CI", "Significance"])
print("\n=== Multiple Regression Coefficients Table ===")
print(multi_df.to_string(index=False))

# ===== 10. SUMMARY MODEL =====
print("\n=== Model Stats ===")
print(f"R² = {model.rsquared:.3f} | Adjusted R² = {model.rsquared_adj:.3f} | F-stat p = {model.f_pvalue:.4f}")